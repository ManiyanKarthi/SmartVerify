...
router.post('/smart-verify',[
  ...
  if (!errors.isEmpty()) {
     ...
     res.json({ status:422,errors: errors.array() });
	  ...
	  var bill_type =1;
	  console.log('---',response.displayName,'displayName');
	  ...
	  let prediction_data = {date:billed_disp_date,billed_amount:billed_amount,bill_number:bill_transaforms.bill_number,invoice_id:invoice_id};	  
...
router.get('/detect-document',async (req, res, next) => {
	...
	var datapart = await InvoiceModel.InsertInvoice(InvoiceData);
...
router.get('/check_data', async (req,res,next) => {
	...
	if(invoice_res){
		...
		var data_store = invoice_res.document_text_deduction.textAnnotations[0].description;
		//var data_store = invoice_res.data.textAnnotations[0].description;
